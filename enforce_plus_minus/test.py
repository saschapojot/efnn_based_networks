...
num_layers = 3
A=1
model = self_similar_model(input_dim, output_dim, num_layers, A).to(device)
...
# 5. Evaluate the model and calculate split MSE
with torch.no_grad():
    predictions, _ = model(X_test_tensor)

    # Compute Total Test MSE
    mse_loss_total = torch.mean(torch.abs(predictions - Y_test_tensor.to(torch.cfloat)) ** 2)
    print(f"Total Test MSE Loss: {mse_loss_total.item():.6e}")

    if has_train_bounds:
        # Create masks for interpolation and extrapolation
        # Squeeze tensors to 1D to ensure masks align properly
        x_flat = X_test_tensor.squeeze()
        interp_mask = (x_flat >= train_min_x) & (x_flat <= train_max_x)
        extrap_mask = ~interp_mask

        # Calculate Interpolation MSE (Within training bounds)
        if interp_mask.any():
            preds_interp = predictions[interp_mask]
            targets_interp = Y_test_tensor.to(torch.cfloat)[interp_mask]
            mse_interp = torch.mean(torch.abs(preds_interp - targets_interp) ** 2)
            print(f"Interpolation MSE Loss (inside training range):  {mse_interp.item():.6e}")
        else:
            print("Interpolation MSE Loss: N/A (No test data inside training range)")

        # Calculate Extrapolation MSE (Outside training bounds)
        if extrap_mask.any():
            preds_extrap = predictions[extrap_mask]
            targets_extrap = Y_test_tensor.to(torch.cfloat)[extrap_mask]
            mse_extrap = torch.mean(torch.abs(preds_extrap - targets_extrap) ** 2)
            print(f"Extrapolation MSE Loss (outside training range): {mse_extrap.item():.6e}")
        else:
            print("Extrapolation MSE Loss: N/A (No test data outside training range)")
